...
class LaSDI:
    ...
    def train_dynamics(self, ls_trajs, training_values, dt, normal = 1, degree = 1, include_interaction=False, LS_vis = True, threshold = 0, alpha = 0, sampling_rate = 1):
        """
        ...
        if self.Local == False: # Global approach
            ...
            model.fit(data_LS, t = sampling_rate*dt, multiple_trajectories = True)
            self.model = model
            if LS_vis == True:
                if self.NN == True:
                ...
                new = model.simulate(data_LS[-1][0], np.linspace(0, dt*(len(data_LS[-1])-1), len(data_LS[-1])))
                for dim in range(data_LS[-1].shape[1]):
        ...
        if self.Local == False: # Global approach
            ...
            if self.Coef_interp == False: # WITHOUT SINDy coefficient interpolation
                ...
                if self.LS_vis == True:
                    ...
                    new = model.simulate(local[-1][0], np.linspace(0, self.dt*(len(local[-1])-1), len(local[-1])))
                    #print(data_LS[-1][0])
